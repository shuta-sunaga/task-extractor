...
export async function POST(
  ...
  try {
    ...
    console.log('[Lark Webhook] Received request for token:', token.substring(0, 8) + '...')

    // トークンから企業を特定
    const company = await getCompanyByWebhookToken(token)
    if (!company) {
      console.error('[Lark Webhook] Invalid webhook token')
      return NextResponse.json({ error: 'Invalid webhook token' }, { status: 404 })
    }

    const companyId = company.id
    console.log('[Lark Webhook] Company identified:', company.name, 'ID:', companyId)
    ...
    if (!settings?.lark_verification_token) {
      console.error('[Lark Webhook] Verification token not configured for company:', companyId)
