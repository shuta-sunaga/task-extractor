...
- 完了タスクの一括削除機能を追加
- 企業管理者のみ使用可能
- 確認モーダルで「削除します」と入力しないとボタンが押せない仕様

#### 実施内容
1. **一括削除API** (`app/api/tasks/completed/route.ts`) - 新規
   - `DELETE /api/tasks/completed`
   - 企業管理者（userType === 'admin'）のみ実行可能
   - 自社の完了タスクのみ削除

2. **DB関数追加** (`lib/db.ts`)
   - `deleteCompletedTasks(companyId)`: 指定企業の完了タスクを一括削除

3. **ダッシュボード更新** (`app/[slug]/page.tsx`)
   - 管理者かつ完了タスクがある場合のみ「完了タスクを一括削除」ボタン表示
   - 削除件数をボタンに表示

4. **確認モーダル実装**
   - 警告アイコン付きヘッダー
   - 削除対象の件数を表示
   - テキストボックスに「削除します」と入力しないと削除ボタンが無効
   - 削除中はキャンセル不可

#### 備考
- 一般ユーザーには削除ボタンが表示されない
- 誤操作防止のため、確認テキスト入力を必須化
- 削除完了後にアラートで件数を表示
